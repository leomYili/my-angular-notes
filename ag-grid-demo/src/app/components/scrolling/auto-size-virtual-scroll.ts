...
export class AutoSizeVirtualScrollStrategy implements VirtualScrollStrategy {
  ...
  private _updateTotalContentSize(renderedContentSize: number) {
    const viewport = this._viewport!;
    const renderedRange = viewport.getRenderedRange();
    console.log(renderedRange,viewport);
    const totalSize =
      renderedContentSize +
 ...
 */
export function _autoSizeVirtualScrollStrategyFactory(
  autoSizeDir: CdkAutoSizeVirtualScroll
) {
...
@Directive({
  selector: 'cdk2-virtual-scroll-viewport[autosize]',
  providers: [
    {
